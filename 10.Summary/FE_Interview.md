...
- 拥塞避免：将拥塞窗口的增长变为线性增长，无论是在慢启动阶段还是拥塞避免阶段，一旦网路出现拥塞，就把慢启动门限设置为窗口大小的一半，然后窗口大小设置为1，之后执行慢启动算法；
- 快速重传：当接收方受到一个失序的数据时，会发送一个重复确认的信号，当发送方收到三个重复确认的信号时，就会立即对这个数据进行重传，而不会等到重传计时器时间到期。
- 快速恢复：当接收方受到三个重传信号时，会将慢启动门限设置为拥塞窗口大小的一半，考虑到网络可能没有出现拥塞，所以此时不执行慢启动算法，而是将拥塞窗口的大小设置为慢启动门限的大小，然后使用拥塞避免算法。
...
- src 表示的是对资源的引用，它会将指向的资源下载并嵌入到标签所在位置，比如 img、script 标签，会将图片嵌入到标签所在位置；浏览器在解析到这些资源的时候，会暂定对其他资源的处理，直到该资源加载执行完毕；
...
1. hash 模式
   ...
   - 可以监听 hashchange 事件监听 hash 的变化，从而控制页面的跳转。
2. history 模式
