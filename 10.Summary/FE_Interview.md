...
- 拥塞避免：将拥塞窗口的增长变为线性增长，一旦网路出现拥塞，就把慢启动门限设置为窗口大小的一半，然后窗口大小设置为1，之后执行慢启动算法；
- 快速重传：当接收方受到一个失序的数据时，会发送一个重复确认的信号，当发送方收到三个重复确认的信号时，就会立即对这个数据进行重传，而不会等到重传计时器时间到期。
- 快速恢复：当接收方受到三个重传信号时，会将慢启动门限设置为原来的一半，考虑到网络可能没有出现拥塞，所以此时不执行慢启动算法，而是将拥塞窗口的大小设置为慢启动门限的大小，然后使用拥塞避免算法。
