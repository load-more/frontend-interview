...
5.  class 的 extends 继承

   extends 后面加上父类class的形式继承，通常需要在 constructor 中调用 super 函数，完成对父类数据的初始化。
...
V8 引擎采用的是分代式垃圾回收。在堆内存中分为新生代空间和老生代空间，新生代采用的是 `Scavenge 算法`，而老生代早先采用的是`引用计数`，后来采用了`标记清除`，之后又改进为`标记整理`。
...
- 浏览器将 hash 值和上一次资源的 hash 值进行比对，比对出差异之后浏览器会向 `webpack-dev-server` 发送 ajax 请求来获取更改的内容（文件列表、hash），之后浏览器根据这些信息继续向 `webpack-dev-server` 发送 jsonp 请求获取该 chunk 的增量更新；
- 之后 `HotModulePlugin` 提供了一些 API 处理这些增量更新，比如 `react-hot-loader` 和 `vue-loader` 都是借助这些 API 实现 HMR。
